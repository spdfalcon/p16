...
export default function DeleteModal({ submitAction, cancelAction ,isShowAcceptModal , isSowDeleteModal, title }) {
    return ReactDOM.createPortal(
        <div className={`w-full h-screen top-0 fixed bg-gray-900/60 flex justify-center items-center font-medium font-lalezar duration-500  ${isSowDeleteModal || isShowAcceptModal ? `opacity-100 visible` : `opacity-0 invisible `}`}>
            <div className='bg-gray-200 px-20 relative py-16 rounded-md flex flex-col gap-5'>
                <div><h2 className='text-xl'>{title}</h2></div>
